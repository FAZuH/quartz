...
Misalkan $X_{1},X_{2},\dots,X_{n}$ menyatakan [[Def-random-sample|sampel random]] berukuran $n$ dari suatu distribusi bertipe kontinu dengan [[Def-probability-density-function-(pdf)|p.d.f.]] $f(x)$ yang positif pada $a<x<b$ dan sama dengan nol untuk $x$ yang lain.

Misalkan $Y_{1}=\min(X_{1},X_{2},\dots,X_{n}),Y_{2}=X_{i}$ berikutnya sesuai dengan urutan besarnya, $\dots$, dan $Y_{n}=\max(X_{1},X_{2},\dots,X_{n})$, maka $Y_{i}=i=1,2,\dots,n$ disebut [[Def-order-statistics|statistik terurut]] ke-$i$ dari sampel random $X_{1},X_{2},\dots,X_{n}$ dan [[mathstat4.4#Theorem 4.4.1 Joint pdf of order statistics|p.d.f. bersama]] dari $Y_{1},Y_2,\dots,Y_{n}$ adalah
