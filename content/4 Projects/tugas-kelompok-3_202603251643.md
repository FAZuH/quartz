...
aliases:
  - Tugas Kelompok 3
created: 2026-03-29T12:54:01.186+07:00
modified: 2026-04-06T16:38:32.950+07:00
published: 2026-04-06T16:38:32.950+07:00
tags:
  - 
cssclasses: ""
...
[[3 Reference/def-survival-function_202603281500\|Survival function]] at time $t$ can be estimated with [[3 Reference/def-kaplan-meier-estimator_202603281500\|Kaplan-Meier Estimator]] using:

$$
\hat{S}(t) = \prod_{t_{i}\leq t} \left( 1- \frac{d_{i}}{Y_{i}}\right)
$$

where:
- $\hat{S}(t)$ : Estimated [[3 Reference/def-survival-time_202603281500\|survival time]] at time $t$
- $d_{i}$ : Event <u>occuring</u> at time $t_{i}$
- $Y_{i}$ : Event <u>remaining</u> at time $t_{i}$

Let 
- $c_{i}$ : [[3 Reference/def-censored-data_202603281500\|Censored data]] at time $t_{i}$
- $Y_{1}=25$
- $Y_i = Y_{i-1} - d_{i-1} - c_{i-1}$

Applying this estimator to the data:

| $t_{i}$ | $d_{i}$ | $c_{i}$ | $Y_{i}$ | $\left(1 - \dfrac{d_{i}}{Y_{i}}\right)$ | $\hat{S}(t_{i})$ |
| ------- | ------- | ------- | ------- | --------------------------------------- | ---------------- |
...
The [[3 Reference/Def-variance\|variance]] and [[3 Reference/standard-error_202509231153\|standard error]] of the estimated survival function can also be estimated using [[3 Reference/def-greenwood-formula_202603281600\|Greenwood's Formula]]:
...
Continuing from point [[4 Projects/tugas-kelompok-3_202603251643#a.]],

Let 
$$
v_{i} = 
\begin{cases}
\dfrac{d_{i}}{Y_{i}(Y_{i-d_{i}})}, & Y_{i} \neq 0 \\
0, & Y_{i} = 0
\end{cases}
$$
...
| $t_i$ | $d_i$ | $Y_i$ | $\hat{S}(t_i)$ | $v_i$  | $\sum_{t_i \leq t} v_i$ | $[\hat{S}(t_i)]^2$ | $\widehat{\text{Var}}[\hat{S}(t_i)]$ | $\widehat{\text{SE}}[\hat{S}(t_i)]$ |
| ----- | ----- | ----- | -------------- | ------ | ----------------------- | ------------------ | ------------------------------------ | ----------------------------------- |
