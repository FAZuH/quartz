...
A [[3 Reference/def-transition-probability-matrix_202603280810\|TPM]] $\mathbf{P}$ is **regular** if $\mathbf{P}^k$ has **all positive entries** for some $k > 0$.

A regular chain is necessarily [[3 Reference/def-irreducible_202603280823\|irreducible]] and [[3 Reference/def-period-stochastic_202603280825\|aperiodic]].
...
2. Since $(\mathbf{P}^k)_{ij} > 0$, state $j$ is [[3 Reference/def-accessible-state_202603280820\|accessible]] from state $i$ ($i \to j$).
3. Similarly, since $(\mathbf{P}^k)_{ji} > 0$, state $i$ is [[3 Reference/def-accessible-state_202603280820\|accessible]] from state $j$ ($j \to i$).
4. Because every state can reach every other state, they all belong to the same [[3 Reference/def-communicating-classes_202603280822\|communicating class]].
