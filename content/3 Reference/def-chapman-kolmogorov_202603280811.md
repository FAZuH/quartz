...
The **Chapman-Kolmogorov Equation** gives the [[3 Reference/def-transition-probability_202603280807\|transition probability]] to go from [[3 Reference/def-state-stochastic_202603280801\|state]] $i$ to $j$ in $n + m$ steps:

$$P_{ij}^{(n+m)} = \sum_{k=0}^{N} P_{ik}^{(n)} P_{kj}^{(m)}$$

Where $N$ is the number of all [[3 Reference/def-state-stochastic_202603280801\|states]].
