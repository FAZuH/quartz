...
> Let $X$ have a [[Continuous Distributions#Gamma distribution|Gamma distribution]] with $\alpha=4$ and $\beta=\theta>0$.
> a. Find the [[def-fisher-information_202507170919|Fisher Information]] $I(\theta)$.
> b. If $X\_{1},X\_{2},\dots,X\_{n}$ is a [[Def-random-sample|random sample]] from this distribution, show that the [[Def-maximum-likelihood-estimator-(mle)|m.l.e.]] of $\theta$ is an [[def-efficient-estimator_202507170919|efficient estimator]] of $\theta$.
...
Fungsi log-[[Def-likelihood-function|likelihood]] untuk satu pengamatan $X$ adalah:
$$
\begin{align}
...
$$ \ln L(\theta) = \sum\_{i=1}^n \ln f(x\_i;\theta) = \sum\_{i=1}^n \left(-\ln 6 - 4\ln\theta + 3\ln x\_i - \frac{x\_i}{\theta}\right) = -n\ln 6 - 4n\ln\theta + 3\sum\_{i=1}^n \ln x\_i - \frac{1}{\theta}\sum\_{i=1}^n x\_i $$

Sehingga didapati
$$
\begin{align}
\frac{\partial \ln L}{\partial \theta} & = -\frac{4n}{\theta} + \frac{1}{\theta^2}\sum\_{i=1}^n x\_i= 0 \\
& \iff \frac{1}{\theta^2}\sum\_{i=1}^n x\_i = \frac{4n}{\theta} \\
& \iff \hat{\theta} = \frac{\sum X\_i}{4n} = \frac{\bar{X}}{4}
\end{align}
$$
...
[[def-rao-cramer-lower-bound_202507171109|Rao-Cramer Lower Bound]] untuk penaksir tak bias dari $\theta$ adalah:
$$ \frac{1}{nI(\theta)} = \frac{1}{n(4/\theta^2)} = \frac{\theta^2}{4n} $$

Karena $\operatorname{Var}(\hat{\theta})=\dfrac{1}{nI(\theta)}$, maka berdasarkan [[mathstat6.2#Corollary 6.2.1 Rao-Cramér bound for unbiased estimators|Corollary 6.2.1 Rao-Cramér bound for unbiased estimators]], $\dfrac{\theta^2}{4n}$ adalah penaksir efisien untuk $\theta$.
...
Diketahui [[Def-probability-density-function-(pdf)|pdf]] dari [[Continuous Distributions#Normal distribution|distribusi normal]] $N(0,\theta)$ adalah
$$
\begin{align}
...
[[def-fisher-information_202507170919|Fisher information]] untuk $n$ sampel adalah:
$$
\begin{align}
I\_n(\theta) & = nI(\theta) \\
...
Untuk sampel $X\_1, X\_2, \dots, X\_n$, log-likelihood adalah:
$$
\ln L(\theta) = \sum\_{i=1}^{n} \ln f(X\_i;\theta) = -n\ln\theta - \frac{n}{2}\ln(2\pi) - \frac{1}{2\theta^2}\sum\_{i=1}^{n}X\_i^2
$$

Turunan pertama terhadap $\theta$:
$$
\begin{align}
\frac{d}{d\theta}\ln L(\theta) & = -\frac{n}{\theta} + \frac{1}{\theta^3}\sum\_{i=1}^{n}X\_i^2 \\
0 & = -\frac{n}{\theta} + \frac{1}{\theta^3}\sum\_{i=1}^{n}X\_i^2 \\
\frac{n}{\theta} & = \frac{1}{\theta^3}\sum\_{i=1}^{n}X\_i^2 \\
n\theta^2 & = \sum\_{i=1}^{n}X\_i^2 \\
\theta^2 & = \frac{1}{n}\sum\_{i=1}^{n}X\_i^2
\end{align}
$$

Berdasarkan definisi [[Def-maximum-likelihood-estimator-(mle)|MLE]] untuk $\theta$ adalah
$$
...
$$
\operatorname{Var}(Y) = \operatorname{Var}\left(\sqrt{\frac{1}{n}\sum\_{i=1}^{n}X\_i^2}\right) = \frac{\theta^2}{2n}
$$

Andaikan berlaku [[mathstat6.1#Assumptions 6.1.1 Regularity conditions|asumsi regularitas]] dan [[mathstat6.2#Assumptions 6.2.1 Additional regularity conditions 1|asumsi regularitas tambahan 1]] berlaku, maka [[def-rao-cramer-lower-bound_202507171109|Rao-Cramer Lower Bound]] adalah
$$
\text{CRLB} = \frac{1}{I\_n(\theta)} = \frac{\theta^2}{2n}
$$

Karena $\operatorname{Var}(\hat{\theta}) = \text{CRLB}$, maka $\hat{\theta}$ adalah [[def-efficient-estimator_202507170919|efficient estimator]] dari $\theta$.

## 8.27

> If $X\_{1},X\_{2},\dots,X\_{n}$ is a [[Def-random-sample|random sample]] from a distribution with [[Def-probability-density-function-(pdf)|p.d.f.]] $f(x;\theta)=3\theta^3(x+\theta)^{-4}$, $0\<x<\theta$, zero elsewhere, where $0<\theta$, show that $Y=2X$ is an [[Def-unbiased-estimator|unbiased estimator]] of $\theta$ and determine its [[mathstat6.2#^def-efficiency|efficiency]].
...
$$
\begin{align}
E(X) & = \int\_0^\infty x \cdot 3\theta^3(x+\theta)^{-4} dx \\
& = 3\theta^3 \int\_\theta^\infty (u-\theta)u^{-4} du  \\
& = 3\theta^3 \int\_\theta^\infty (u^{-3} - \theta u^{-4}) du  \\
...
E\[X^2] & = \int\_0^\infty x^2 \cdot 3\theta^3(x+\theta)^{-4} dx  \\
& = 3\theta^3 \int\_\theta^\infty (u-\theta)^2 u^{-4} du  \\
& = 3\theta^3 \int\_\theta^\infty (u^{-2} - 2\theta u^{-3} + \theta^2 u^{-4}) du  \\
& = \theta^2 \ \\
...
$E\left\[\frac{1}{(X+\theta)^2}\right] = \int\_0^\infty \frac{3\theta^3}{(x+\theta)^6} dx = \frac{3}{5\theta^2}$
...
> If $X\_{1},X\_{2},\dots,X\_{n}$ is a [[Def-random-sample|random sample]] from $N(\theta,1)$, find a lower bound of the [[Def-variance|variance]] of an estimator of $k(\theta)=\theta^2$. Determine an [[def-minimum-variance-unbiased-estimator-(mvue)_202507170925|unbiased minimum variance estimator]] of $\theta^2$ and then compute its [[mathstat6.2#^def-efficiency|efficiency]].
...
Diketahui $\bar{X}\sim N(\theta,1/n)$ adalah [[def-complete-sufficient-statistic_202507170931|statistik cukup yang komplit]] untuk $\theta$
...
\end{align}
$$
Sehingga, $E\[\bar{X}^2 - 1/n] = \theta^2$. Artinya, $T = \bar{X}^2 - 1/n$ adalah penaksir tak bias untuk $\theta^2$. Berdasarkan teorema [[unique-mvue-(umvue)_202507170937|Lehmann Scheffe]], $T$ adalah MVUE untuk $\theta^2$.
