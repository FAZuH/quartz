...
> $f(x^_) \geq f(x)$ for all $x \in A$.
> We say that $f$ has an absolute minimum on $A$ if there is a point $x\__ \in A$ such that
> $f(x\_\*) \leq f(x)$ for all $x \in A$.
> We say that $x^$ is an absolute maximum point for $f$ on $A$, and that $x\_$ is an absolute minimum point for $f$ on $A$, if they exist.
