---
publish: true
aliases:
  - Bucket Sort
created: 2026-04-09T23:07:39.023+07:00
modified: 2026-04-09T23:07:39.024+07:00
published: 2026-04-09T23:07:39.024+07:00
creation-time: 2025-08-18 02:11
status: in progress
tag:
parent: "[[sorting-algorithms_202508172342]]"
---

1. Create an array of buckets
2. Assign each element to one bucket based on its value (or function of)
3. Sort each bucket. Usually [[insertion-sort_202508171730|Insertion Sort]] due to it being faster for small arrays

> [!note]
> This [[sorting-algorithms_202508172342|sorting algorithm]] is efficient if you know the distribution of the elements beforehand, and design the bucket assignment rule based on that

See https://www.youtube.com/watch?v=VuXbEb5ywrU

## Implementation

```python
class BucketSort(Sort[int]):
    def __init__(self, buckets: int, sorting_method: Sort[int]) -> None:
        self.buckets = buckets
        self.sorting_method = sorting_method
        super().__init__()

    def sort(self, array: list[int]) -> list[int]:
        # ASSUMPTION: array consists of integers from 0 to 10

        if len(array) <= 1:
            return array
        # INVARIANT: array has 2 or more elements

        buckets: list[list[int]] = [list() for _ in range(self.buckets)]

        _max = max(array)
        _min = min(array)
        for n in array:
            # NOTE: Scale numbers from 0 to 10 to 0 to self.buckets - 1
            scaled_n = (self.buckets - 1) * (n - _min) // (_max - _min)
            buckets[scaled_n].append(n)

        ret: list[int] = []
        for bucket in buckets:
            ret.extend(self.sorting_method.sort(bucket))
        return ret
```
