...
- $\mathcal B$ : [[Def-events|Events]] ($\mathcal{B}\subset \mathcal{C}$)
...
2. $P(\mathcal C) = 1$
3. If ${ A\_{n} }\in \mathcal B$ and $A\_{m}\cap A\_{n}=\phi,;\forall m\neq n$ then
   $$
   P\left(\bigcup\_{n=1}^{\infty} A\_n\right)=\sum\_{n=1}^{\infty} P\left(A\_n\right)
   $$
...
A probability [[def-set_202510022014|set]] function is essentially a function that maps [[def-subset_202507240733|subset]] of events $\mathcal{B}$ to a real number.
