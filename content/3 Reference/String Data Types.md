...
Like [[#CHAR and VARCHAR types]] but stores **strings as binaries**.
...
Unlike [[#CHAR and VARCHAR types]], there is no padding on insert and no bytes are stripped on select.

If `TEXT` is indexed and the _index require unique values_, duplicate-key **errors occur for values that differ only in number of trailing spaces**. This is not true for `BLOB` columns.
...
- Only the first [max\_sort\_length](https://dev.mysql.com/doc/refman/8.4/en/server-system-variables.html#sysvar_max_sort_length) bytes of the column are used when sorting. Defaults to 1024.
