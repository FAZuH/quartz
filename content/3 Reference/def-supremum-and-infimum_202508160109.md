...
creation-time: 2025-08-16 01:09
status: in progress
tag:
...
1. If $S$ is bounded above, then a number $u$ is said to be a **supremum** (or a **least upper bound**) of $S$ if it satisfies the conditions: ^a
   1. $u$ is an [[def-bounded-set_202508160108|upper bound]] of $S$, and ^b
...
If $u\_1 = \sup S$ and $u\_2 = \sup S$, it must be that $u\_1 = u\_2$.
...
> - In order to show that $u = \sup S$ for some nonemtpy subset $S$ of $\mathbb R$, we need to show **[[#^b|(1)]] and [[#^c|(2)]] of [[#^a|2.3.2 (1)]]** holds.
>   This can also be done using [[#2.3.3 Lemma]] or [[#2.3.4 Lemma]].
...
>   3. If $z< u$, then there exists $s\_z \in S$ such that $z < s\_z$.
>   4. If $\varepsilon > 0$, then there exists $s\_\varepsilon \in S$ such taht $u - \varepsilon < s\_\varepsilon$.
...
> [!NOTE]
> This is the negation of [[#^c|(2) of 2.3.2 (1)]].
...
>
> 1. $s \leq u, \forall s \in S$
> 2. $\forall \varepsilon > 0$, there exists an $s\_\varepsilon \in S$ such that $u - \varepsilon < s\_\varepsilon$
