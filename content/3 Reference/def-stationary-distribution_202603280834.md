...
> [!TIP] Interpretation
> If the initial [[def-state-stochastic_202603280801|state]] $X_0$ is chosen according to $\pi$, then the probability of being in state $j$ at any time $n$ is also $\pi_j$ (i.e., $P(X_n = j) = \pi_j$ for all $n$).

## Properties

- A stationary distribution may exist without a limiting distribution
- For an [[def-irreducible_202603280823|irreducible]] [[def-ergodic-state_202603280835|ergodic]] Markov chain, the limiting distribution always exists and equals the unique stationary distribution
