---
{"creation-time":"2025-03-13 06:40","status":"elder","tags":null,"parent":["[[discriminant analysis]]"],"publish":true,"PassFrontmatter":true}
---
...
Imagine MDA as **drawing lines on a scatterplot to separate clusters**. Each cluster is a group (e.g., apples, oranges, bananas), and each point is an observation (e.g., a fruit’s weight and color). One line might split apples from the rest, but a second is needed to separate oranges from bananas.

- **Number of Functions**: Up to $\min(G-1, p)$, where $G$ is groups and $p$ is variables. For 3 groups and 2 variables, max is 2 functions.
- **Goal**: Maximize separation between groups while keeping within-group spread tight.
...
![mda_fruit_manual.png|500](../Assets/mda_fruit_manual.png)
